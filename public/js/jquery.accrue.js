...
;(function( $, window, document, undefined ){
    ...
    $.fn.accrue.options = {
        ...
        field_titles: {
            ...
            rate: "Tasa (TAE)",
            rate_compare: "Tasa de comparación",
            term: "Término (m=meses, a=años)"
        },
    ...
    var get_field = function( elem, options, name ) {
        ...
        elem.find(".form").append(
            '<div class="col-md-3 accrue-field-'+name+'">'+
                '<p><label>'+options.field_titles[name]+':</label>'+
                '<input type="text" class="form-control '+name+'" name="'+name+'" value="'+options.default_values[name]+'" />'+
    ...
    var calculateAmortization = function( elem, options, output_elem ){
        ...
        if ( loan_info!==0 ) {
            ...
            var output_content = '<table class="accrue-amortization table table-bordered table-striped">'+
                    '<thead><tr>'+
                    '<th class="accrue-payment-number">#</th>'+
                    '<th class="accrue-fecha-pago">Fecha de pago</th>'+
            ...
            var fecha_pago=moment($('#fecha_pago').val());
            
            var diaPago=fecha_pago.date();
            var siguenteMes=fecha_pago;
            ...
            for ( var i=0; i<loan_info.num_payments; i++) { 
                ...
                // config
                var monto_f= formatNumber( loan_info.payment_amount_formatted, options );
                var interest_f= formatNumber( counter_interest.toFixed(2), options );
                var pagos_f= formatNumber( counter_payment.toFixed(2), options );
                var balance_f= formatNumber( counter_balance.toFixed(2), options );
                
                // fechas
                siguenteMes = fecha_pago.add(1, 'M');
                
                var diaPagoSiguenteMes=siguenteMes.date();
                var ultimaFechaSiguenteMes=moment(siguenteMes).endOf('month');
                
                if(diaPago!=diaPagoSiguenteMes){
                    fecha_pago=ultimaFechaSiguenteMes;
                }else{
                    fecha_pago=siguenteMes;
                }
                ...
                output_content = output_content+ 
                    '<tr>'+
                    '<'+cell_tag+' class="accrue-payment-number"><input type="hidden" name="numero_pago['+(i+1)+']" value="'+(i+1)+'" />'+(i+1)+'</'+cell_tag+'>'+
                    '<'+cell_tag+' class="accrue-fecha-pago"><input type="hidden" value="'+fecha_pago.format('YYYY-MM-DD')+'" name="fecha_pago['+(i+1)+']" />'+fecha_pago.format('LL')+'</'+cell_tag+'>'+
                    '<'+cell_tag+' class="accrue-payment-amount"><input type="hidden" value="'+monto_f+'" name="monto_pago['+(i+1)+']" />'+monto_f+'</'+cell_tag+'>'+
                    '<'+cell_tag+' class="accrue-total-interest"><input type="hidden" value="'+interest_f+'" name="interes['+(i+1)+']"/>'+interest_f+'</'+cell_tag+'>'+
                    '<'+cell_tag+' class="accrue-total-payments"><input type="hidden" value="'+pagos_f+'" name="pago_total['+(i+1)+']"/>'+pagos_f+'</'+cell_tag+'>'+
                    '<'+cell_tag+' class="accrue-balance"><input type="hidden" value="'+balance_f+'" name="saldo_capital['+(i+1)+']"/>'+balance_f+'</'+cell_tag+'>'+
                    '</tr>';
